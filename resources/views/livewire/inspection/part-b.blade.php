...
<div wire:key="step-b">
    ...
    @foreach ($formData['operation']['stocking_records'] as $i => $record)
        <div class="grid grid-cols-12 gap-3 items-start mb-4" wire:key="stocking-row-{{ $i }}">
            <div class="col-span-3">
                <flux:input size="sm"
                    wire:model="formData.operation.stocking_records.{{ $i }}.species" />
            </div>
            <div class="col-span-3">
                <flux:input size="sm"
                    wire:model="formData.operation.stocking_records.{{ $i }}.source" />
            </div>
            <div class="col-span-3">
                <flux:input size="sm"
                    wire:model="formData.operation.stocking_records.{{ $i }}.quantity" />
            </div>
            <div class="col-span-3">
                <flux:input placeholder="₱ 0.00" size="sm"
                    wire:model="formData.operation.stocking_records.{{ $i }}.cost" />
            </div>
        </div>
    @endforeach
    ...
    <div class="grid grid-cols-2 gap-x-3 gap-y-2 mb-4">
        ...
        <div>
            ...
            <flux:input placeholder="0.00 kg" size="sm"
                wire:model="formData.operation.harvest_records.kilos_harvested" />
        </div>
        <div>
            <flux:label class="mb-1">Date of Harvest:</flux:label>
            <flux:input type="date" size="sm" wire:model="formData.operation.harvest_records.date_harvest" />
        </div>
        <div>
            <flux:label class="mb-1">Gross Sales:</flux:label>
            <flux:input placeholder="₱ 0.00" size="sm"
                wire:model="formData.operation.harvest_records.gross_sales" />
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-x-3 gap-y-2 mb-4">
        ...
        <div>
            ...
            <flux:input placeholder="0.00 kg" size="sm"
                wire:model="formData.operation.harvest_records.market_domestic_kilos" />
        </div>
        <div>
            <flux:label class="mb-1">Export:</flux:label>
            <flux:input size="sm" wire:model="formData.operation.harvest_records.market_export" />
        </div>
        <div>
            <flux:label class="mb-1">No. of Kilos:</flux:label>
            <flux:input placeholder="0.00 kg" size="sm"
                wire:model="formData.operation.harvest_records.market_export_kilos" />
        </div>
    </div>
</div>
