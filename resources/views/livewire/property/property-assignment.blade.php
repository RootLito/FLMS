...
new class extends Component {
    ...
    public function saveMap($coordinates)
    {
        ...
        $lessee->fishpondMap()->updateOrCreate(['lessee_id' => $this->selectedLesseeId], ['coordinates' => $coordinates]);
...
<div class="flex-1 flex flex-col h-screen overflow-hidden" x-data="{ selectedId: @entangle('selectedLesseeId') }">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="grid grid-cols-2 gap-2 mt-3 text-xs">
                                    <div class="flex flex-col">
                                        ...
                                        <span
                                            class="text-neutral-700 dark:text-neutral-300 font-mono">{{ $lessee->fla_no }}</span>
                                    </div>
    ...
    <flux:modal name="draw-map-modal" class="fixed inset-0! max-w-[900px]! h-screen! w-screen! p-0!"
        x-data="editorMap">
        <div class="flex flex-col h-full">
            ...
            <div class="h-full min-h-0 w-full relative">
                <div id="editor-canvas" class="w-full h-full bg-zinc-100 dark:bg-zinc-900" wire:ignore
                    x-init="observeVisibility($el)"></div>
            </div>
    ...
    <script>
        ...
        Alpine.data('editorMap', () => ({
            ...
            initMap(container) {
                ...
                this.map = L.map(container).setView([8.0, 124.5], 8);
                ...
                if (existing && existing.length > 0) {
                    this.activeLayer = L.polygon(existing, {
                        color: '#3b82f6'
                    }).addTo(this.map);
